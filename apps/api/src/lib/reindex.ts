...
import { embedText, embedTexts, toPgVector } from "./embeddings.js"; // FIXED: Imported embedTexts for batching
...
export const reindexPolicy = async (
  policyId: string,
  ...
  if (!policy) {
    ...
    await pool.query("DELETE FROM document_chunks WHERE policy_id = $1", [
      policyId,
    ]);
    return { chunkCount: 0 };
  }
  ...
  try {
    ...
    await client.query("DELETE FROM document_chunks WHERE policy_id = $1", [
      policyId,
    ]);
...
export const retrieveChunks = async (
  ...
  const sortedCandidates = [...fusionScores.entries()].sort(
    (a, b) => b[1] - a[1],
  );
  ...
  // Pass 1: Grab up to 2 best chunks per unique policy
