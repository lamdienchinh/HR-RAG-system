...
export const PolicyCreator = ({
  ...
  return (
    <div className="shrink-0 space-y-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      ...
      <input
        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-100"
        disabled={disabled}
      ...
      <Button
        className="w-full"
        size="sm"
        disabled={disabled || title.trim().length === 0 || category.trim().length === 0}
        type="button"
