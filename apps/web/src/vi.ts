export const T = {
  // Chat
  welcomeMessage:
    "Xin chào! Hãy hỏi tôi về chính sách nhân sự. Tôi sẽ truy xuất bằng chứng, chấm điểm và chỉ trả lời khi độ tin cậy đạt ngưỡng bạn cài đặt.",
  ...
  minScoreExplanation:
    "Nếu tất cả chunk truy xuất có điểm dưới giá trị này, trợ lý sẽ từ chối trả lời thay vì đưa ra câu trả lời không chính xác.",
  googleSearch: "Google Search tham khảo",
  googleSearchEnabled:
    "Tùy chọn Google Search grounding khi bằng chứng nội bộ yếu. Câu trả lời được đánh dấu là tham khảo.",
  ...
  policyIndexed: (chunks: number, policies: number) =>
    `Đã đánh chỉ mục ${chunks} chunk từ ${policies} chính sách. Chat đã cập nhật.`,
  policyCreating: "Đang tạo chính sách và cập nhật vector...",
  policyCreated: (title: string, chunks: number) =>
    `Đã tạo "${title}" và đánh chỉ mục ${chunks} chunk.`,
  policyDeleting: "Đang xóa chính sách và cập nhật vector...",
  policyDeleted: (title: string, chunks: number) =>
    `Đã xóa "${title}" và đánh chỉ mục ${chunks} chunk còn lại.`,
